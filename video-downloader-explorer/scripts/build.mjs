...
async function buildOffscreen() {
  await build({
    ...
    build: {
      ...
      emptyOutDir: false,
      lib: {
        entry: path.join(root, "src/background/offscreen.ts"),
        formats: ["iife"],
        name: "VideoDownloaderOffscreen",
...
async function copyAssets() {
  ...
  await copyFile(path.join(root, "offscreen.html"), path.join(root, "dist/offscreen.html"));
  ...
  // Удаляем sourcemap'ы (если vite создал).
  for (const m of ["background.js.map", "content.js.map"]) {
    await rm(path.join(root, "dist", m), { force: true });
  }
}
