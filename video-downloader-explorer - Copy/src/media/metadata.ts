...
export async function resolveVideoMetadata(
  ...
  // YouTube-ссылки содержат clen=…, mime=video/mp4, dur=… — они валидны даже когда
  // HEAD/Range заблокированы (403) и формат иначе остался бы unknown.
  ...
  if (!fileSize || !mimeType) {
    try {
      ...
      if (!fileSize && clen) {
        fileSize = clen;
        metadataLog.debug(`Fallback: clen=${clen} из query URL`);
      }
