...
/** Уникальный ключ кандидата по URL (без query, для группировки вариантов). */
export function variantGroupKey(videoUrl: string): string {
  ...
  try {
    const u = new URL(n);
    u.hash = "";
    // Только path — варианты с разными ?quality=… — одно и то же видео.
...
export function applyFilenameTemplate(
  ...
  const safe: Record<string, string> = {
    title: sanitizeFilename(vars.title || "video"),
    domain: sanitizeFilename(vars.domain || "site"),
    ...
    quality: sanitizeFilename(vars.quality || ""),
    ext: (vars.ext || "mp4").toLowerCase(),
    format: sanitizeFilename(vars.format || ""),
    date: new Date().toISOString().slice(0, 10),
  };
  return template.replace(/\{(\w+)\}/g, (_, k: string) => safe[k] ?? "");
}
