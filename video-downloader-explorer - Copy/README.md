...
- **DASH** (mpd) — парсинг Period/AdaptationSet/Representation
...
- **Cookies/session** — НЕ передаются по умолчанию. Включается вручную в настройках (`useCookies: true`).
- **Referer** — нативный. Подмена через `declarativeNetRequest` не реализована в MVP (Phase 2).
...
- DASH mux (сейчас — раскрытие и загрузка init, не склейка сегментов)
- WebM/MKV mux
- Smooth Streaming (ISM)
- MSE-сниффинг (перехват сегментов SourceBuffer)
