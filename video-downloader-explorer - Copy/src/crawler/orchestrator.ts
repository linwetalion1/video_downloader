...
export class Crawler {
  ...
  enqueueDownloads(ids?: string[]): void {
    ...
    for (const c of list) {
      ...
      if (c.isBlob) {
        c.status = "skipped";
        c.phase = "done";
        c.message = "blob (MSE) — файла нет; выберите прямые mp4/HLS-ссылки из списка";
        this.stats.totalSkipped++;
        skippedBlobs++;
        this.deps.onEvent({ type: "log", entry: downloadLog.warn(`SKIP blob: ${c.videoUrl.slice(0, 60)} — MSE, байтов нет. Рядом в списке есть прямые ссылки.`) });
        this.emitCandidateUpdate(c.id, { status: c.status, phase: c.phase, message: c.message });
        continue;
      }
      const task: Task = {
  ...
  cancelDownloads(ids?: string[]): void {
    if (!ids) {
      ...
      this.deps.onEvent({ type: "log", entry: downloadLog.warn(`Очередь очищена (${removed} задач)`) });
      return;
    }
    for (const id of ids) {
      const c = this.candidates.get(id);
